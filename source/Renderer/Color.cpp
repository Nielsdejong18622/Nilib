...
{
    RGBf(R, G, B);
    *A = store.rgba[3] / 255.0f;
}
void Color::RGBf(float *R, float *G, float *B) const
{
    *R = store.rgba[0] / 255.0f;
    *G = store.rgba[1] / 255.0f;
    *B = store.rgba[2] / 255.0f;
}
...
{
    return Color::HSL(RNG::prob() * 360.0f, RNG::uniform(0.5f, 0.8f), RNG::uniform(0.5f, 0.8f), 200U);
}
