...
class Argparser
{
  public:
    ...
    void check()
    {
        ...
        size_t user_parsed = static_cast<size_t>(
            std::count_if(d_user.begin(), d_user.end(), [](UserArg const &arg) { return arg.parsed; }));
        size_t command_parsed = static_cast<size_t>(std::count_if(
            d_commandline.begin(), d_commandline.end(), [](CommandLineArg const &arg) { return arg.parsed; }));
    ...
    template <typename Type> void argument(Type &store, std::string_view argc_name, std::string_view const description)
    {
  ...
  private:
    ...
    // Utility functions.
    template <typename Type> size_t find(std::vector<Type> const &search, Type const &key)
    {
    ...
    {
        auto pred = [&prefix](CommandLineArg const &type) { return type.data.starts_with(prefix); };
        return static_cast<size_t>(std::distance(std::begin(search), std::find_if(start, std::end(search), pred)));
    }

    template <typename Type> bool contains(std::vector<Type> const &search, Type const &key)
    {
    ...
    template <typename Type> bool equality_arg(CommandLineArg const &shorthand_eq, Type &store)
    {
    ...
    template <typename Type> bool space_arg(CommandLineArg const &arg, Type &store)
    {
