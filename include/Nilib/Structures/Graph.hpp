...
public:
    ...
    node_id find(NodeData const &data) const {
        auto lambda = [data](NodeBlock const &b) { return (b.data == data && b.occupied); };
    ...
    std::vector<node_id> nodes() const {
        ...
        {
            if (!d_nodes[i].occupied) continue;
            res.emplace_back(i);
        }
        return res;
    }
    ...
    node_id addNode(NodeData const &node)
    {
        ...
        if (d_fragmented > 0) 
        {
            ...
            auto avail = std::find_if(d_nodes.begin(), d_nodes.end(), [](NodeBlock &b) { return !b.occupied; });
    ...
    std::vector<node_id> down_stream(node_id const idx) const 
    {
        ...
        if (begin == d_edges.end()) return std::vector<node_id>();
