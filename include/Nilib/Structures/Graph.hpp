...
namespace Nilib
{
    ...
    public:
        ...
        void removeNode(node_id const idx)
        {
            ...
            ASSERT(d_nodes[idx].occupied, "Error: Node was already removed!")
        ...
        // Breadth first search for source->dest path.
        // Returns empty vector if no path found.
        std::vector<node_id> path(node_id source, node_id dest) const
        {
            std::vector<node_id> res;
            std::unordered_map<node_id, node_id> backtrack;
            auto initfrontier = down_stream(source);
            std::set<node_id> frontier(initfrontier.begin(), initfrontier.back());
            std::set<node_id> visited(source);

            node_id current = source;
            while (!frontier.empty())
            {
                auto nextnode = frontier.extract(0);
                visited.insert(nextnode);
                backtrack[nextnode] = current;
            }

            return res;
        }

        std::vector<node_id> randomWalk(node_id const start, size_t const max_steps) const
        {
    ...
    public:
        ...
        static Graph mst(std::vector<NodeData> const &nodes, std::function<float(NodeData &, NodeData const &)> const &dist_fun)
        {
            Graph<NodeData> g;
            ...
            for (NodeData &&node1 : nodes)
            {
                ...
                auto handle = g.addNode(node1);

                // If this is the first node to be added.
                if (g.d_nodes.size() == 1)
                    continue;

                auto handle_match = handle;
                for (node_id match_id = 0; match_id < g.d_nodes.size(); ++match_id)
                {
                    NodeBlock nb = g.d_nodes[match_id];
                    if (nb.occupied)
                    {
                        float dist = dist_fun(node1, nb.data);
                        if (dist < mindist)
                        {
                            mindist = dist;
                ...
                g.addEdge(handle, handle_match);
                g.addEdge(handle_match, handle);
            }
