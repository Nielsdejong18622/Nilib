...
concept IsDynamicMatrixData =
    std::is_same_v<MatrixData, DynamicMatrixData<ValueType>> || std::is_same_v<MatrixData, SparseMatrixData<ValueType>>;

template <typename value_type = float, typename MatrixData = DynamicMatrixData<value_type>> class Matrix
{
    MatrixData d_data;

  public:
    using scalar = value_type;
    template <typename OtherScalar, typename OtherMatrixData> Matrix(Matrix<OtherScalar, OtherMatrixData> const &other)
    {
    ...
    value_type operator()(size_t const row, size_t const col) const
    {
        return d_data(row * d_data.cols() + col);
    }
    value_type &operator()(size_t const row, size_t const col)
    {
        return d_data(row * d_data.cols() + col);
    }
    value_type &operator()(size_t const idx)
    {
        return d_data(idx);
    }
    ...
    {
        return d_data.rows() * d_data.cols();
    }
    ...
    inline value_type avg() const
    {
        ...
        {
            ++count;
            mean += (d_data(i) - mean) / static_cast<value_type>(count);
        }
        return mean;
    }
    ...
    inline value_type stddev() const
    {
        ...
        for (size_t i = 0; i < d_data.size(); ++i)
        {
            ...
            mean += delta / static_cast<value_type>(count);
            value_type delta2 = x - mean;
            M2 += delta * delta2;
        }

        return std::sqrt(M2 / static_cast<value_type>(count - 1)); // Sample standard deviation
    }
    template <typename Callable> void apply(Callable &&fun)
    {
    ...
    // In place operators.
    template <typename data> void operator+=(Matrix<value_type, data> const &B)
    {
...
template <size_t n, size_t m> using Mat = Matrix<float, StaticMatrixData<n, m, float>>;
...
template <typename scalar> using Vec2 = Matrix<scalar, StaticMatrixData<2, 1, scalar>>;
template <typename scalar> using Vec3 = Matrix<scalar, StaticMatrixData<3, 1, scalar>>;
template <typename scalar> using Vec4 = Matrix<scalar, StaticMatrixData<4, 1, scalar>>;

template <size_t n> using Vecf = Matrix<float, StaticMatrixData<n, 1, float>>;
