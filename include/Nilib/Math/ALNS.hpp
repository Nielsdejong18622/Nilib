...
template <typename Solution> class Solver
{
  public:
    ...
    void solve()
    {
        ...
        {
            threads.emplace_back(&Solver<Solution>::worker, this, t, params.max_iterations, std::ref(global_best),
                                 std::ref(global_best_obj), std::ref(global_mutex));
        }
  ...
  private:
    ...
    constexpr void nullCallback(Solution const &solution) {};

    void worker(size_t thread_id, size_t iterations, Solution &global_best, float &global_best_obj,
                std::mutex &global_mutex)
    {
        // Thread-local copies
        ...
        float const coolingrate = std::pow(0.01f, 1.0f / iterations);
        ...
        for (; iteration < iterations; ++iteration)
        {
            ...
            size_t selected_operator_idx = Nilib::RNG::index(d_operators.size());
        ...
        // Update global best safely
        {
            ...
            LOG_PROGRESS("ALNS solver thread", thread_id, "performed", iteration, "iterations in",
                         timer.getMilliseconds(), "Best objective:", best_obj);
        }
    }
};
