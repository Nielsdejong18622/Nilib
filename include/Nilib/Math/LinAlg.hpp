...
template <typename scalar, typename data>
Matrix<scalar, data> operator*(scalar const lambda, Matrix<scalar, data> const &A)
{
...
Matrix<scalar, data> operator/(Matrix<scalar, data> const &A, scalar const lambda)
{
    return operator*(A, static_cast<scalar>(static_cast<scalar>(1.0) / lambda));
}
template <typename data, typename scalar>
Matrix<scalar, data> operator/(scalar const lambda, Matrix<scalar, data> const &A)
{
...
Matrix<type, DynamicMatrixData<type>> operator*(Matrix<type, DynamicMatrixData<type>> const &A,
...
Matrix<type, StaticMatrixData<n, p, type>> operator*(Matrix<type, StaticMatrixData<n, m, type>> const &A,
                                                     Matrix<type, StaticMatrixData<m, p, type>> const &B)
{
...
Matrix<type, DynamicMatrixData<type>> operator*(Matrix<type, StaticMatrixData<n, m, type>> const &A,
                                                Matrix<type, DynamicMatrixData<type>> const &B)
{
    ASSERT(A.cols() == B.rows(), "A:", A.rows(), "x", A.cols(), "B:", B.rows(), "x", B.cols());
...
template <size_t n, size_t m, typename type>
Matrix<type, DynamicMatrixData<type>> operator*(Matrix<type, DynamicMatrixData<type>> const &A,
                                                Matrix<type, StaticMatrixData<n, m, type>> const &B)
{
    ASSERT(A.cols() == B.rows(), "A:", A.rows(), "x", A.cols(), "B:", B.rows(), "x", B.cols());
...
// Cbind.
template <size_t n, size_t m, size_t p, typename type>
Matrix<type, StaticMatrixData<n + p, m, type>> cbind(Matrix<type, StaticMatrixData<n, m, type>> const &A,
                                                     Matrix<type, StaticMatrixData<p, m, type>> const &B)
{
...
// Cbind [A, \n, B]
template <typename type>
Matrix<type, DynamicMatrixData<type>> cbind(Matrix<type, DynamicMatrixData<type>> const &A,
                                            Matrix<type, DynamicMatrixData<type>> const &B)
{
...
// Rbind.
template <size_t n, size_t m, size_t p, typename type>
Matrix<type, StaticMatrixData<n + p, m, type>> rbind(Matrix<type, StaticMatrixData<n, m, type>> const &A,
                                                     Matrix<type, StaticMatrixData<p, m, type>> const &B)
{
...
template <typename type>
Matrix<type, DynamicMatrixData<type>> rbind(Matrix<type, DynamicMatrixData<type>> const &A,
                                            Matrix<type, DynamicMatrixData<type>> const &B)
{
    CORE_ASSERT(A.rows() == B.rows());
...
template <typename type> type euclidean(Vec2<type> const &A, Vec2<type> const &B)
{
    return std::sqrt(euclidean2(A, B));
}
// Specializations for Vec2
template <typename type> type euclidean2(Vec2<type> const &A, Vec2<type> const &B)
{
...
template <typename type> type euclidean2(Vec3<type> const &A, Vec3<type> const &B)
{
...
{
    return apply(A - B, [](type t) { return std::abs(t); }).sum();
}
...
template <typename value_type, typename type> Matrix<value_type, type> normalize(Matrix<value_type, type> const &A)
{
    auto tmp = A;
    return tmp / A.sum();
}
// Normalize vector.
template <typename type> Matrix<type> normalize(Matrix<type> &A)
{
    return A / A.sum();
}

template <typename value_type, typename type> void normalize(Matrix<value_type, type> &A)
{
    return A / A.sum();
}

template <typename value_type, typename type> void rownormalize(Matrix<value_type, type> &A)
{
...
template <typename value_type, typename type> void colnormalize(Matrix<value_type, type> &A)
{
    for (size_t cdx = 0; cdx < A.cols(); ++cdx)
    {
...
template <typename value_type, typename type> Matrix<value_type, type> colMeans(Matrix<value_type, type> const &A)
{
    ...
    for (size_t col_idx = 0; col_idx < A.cols(); col_idx++)
        for (size_t row_idx = 0; row_idx < A.rows(); row_idx++)
            res(0, col_idx) += A(row_idx, col_idx);
