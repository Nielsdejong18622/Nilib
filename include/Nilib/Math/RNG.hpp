...
namespace Nilib
{
    ...
    public:
        ...
        {
            float const multiplier = static_cast<float>(std::pow(10.0, digits));
            return std::ceil(uniform<float>(min, max) * multiplier) / multiplier;
        }

        // Uniform random variable between [min, max]
        template <typename scalar = float>
        static inline scalar uniform(scalar min, scalar max)
        {
        ...
        template <typename scalar_int>
        static inline scalar_int index(scalar_int size)
        {
            if (size == 0)
                return 0;
            return static_cast<scalar_int>(wyhash(&state) % size);
        }
        ...
        {
            CORE_ASSERT(var > static_cast<scalar>(0.0f))
            return static_cast<scalar>(wy2gau(wyhash(&state))) * std::sqrt(static_cast<scalar>(var)) + mean;
        }
        ...
        {
            return scale * std::pow(-1.0f * std::log(1.0f - RAN<stream>::prob()), 1.0f / shape);
        }
