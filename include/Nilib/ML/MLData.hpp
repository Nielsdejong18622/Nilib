...
template <typename Obs> class MLData
{
  ...
  protected:
    struct Range
    {
        ...
        Range(MLdataiterator b, MLdataiterator e) : d_begin(b), d_end(e)
        {
        }

        MLdataiterator begin() const
        {
            return d_begin;
        }
        MLdataiterator end() const
        {
            return d_end;
        }
    };
  ...
  public:
    ...
    Range train() const
    {
        return Range(d_data.begin(), d_data.begin() + d_valid_start);
    }
    Range validate() const
    {
        return Range(d_data.begin() + d_valid_start, d_data.begin() + d_test_start);
    }
    Range test() const
    {
        return Range(d_data.begin() + d_test_start, d_data.end());
    }

    // Dimensions of the data.
    inline size_t obs() const
    {
        return d_data.size();
    };

    inline void shuffle_training()
    {
        RNG::shuffle(d_data, 0, d_valid_start);
    }
    inline void shuffle_validation()
    {
        RNG::shuffle(d_data, d_valid_start, d_test_start);
    }
