...
class Logger : public BaseLogger
{
  protected:
    ...
    bool endl = true;
    using BaseLogger::operator<<;  // bring all base overloads into scope

  public:
    char sep = ' ';

    Logger(std::ostream &stream) : d_log(LogLevel::Debug), d_stream(stream)
    {
    }
    ...
    template <typename T, typename... Args> void output(T &&first, Args &&...args)
    {
    ...
    template <typename T> constexpr Logger &operator<<(T const &message)
    {
        (d_stream) << message;
        return *this;
    }
